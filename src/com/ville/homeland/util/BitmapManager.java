...
public class BitmapManager {  
    ...
    public void loadBitmap(String url, ImageView imageView, Bitmap defaultBmp, int width, int height) {  
        ...
        if (bitmap != null) {  
        	...
        	//加载SD卡中的图片缓存
        	String filename = createFileNameByUrl(url);
            ...
            public void handleMessage(Message msg) {  
                ...
                if (tag != null && tag.equals(url)) {  
                    if (msg.obj != null) {  
                        ...
                        try {
                        	//向SD卡中写入图片缓存
							ImageUtils.saveImage(imageView.getContext(), createFileNameByUrl(url), (Bitmap) msg.obj);
						} catch (IOException e) {
							e.printStackTrace();
						}
                    } 
	...
	private Bitmap downloadBitmap(String url, int width, int height) {
		Bitmap bitmap = null;
		// http加载图片
		bitmap = NetUtils.getBitmapByUrl(url);
		if (width > 0 && height > 0) {
			// 指定显示图片的高宽
	...
	private String createFileNameByUrl(String url){
		return FileUtils.createFileNameByUrl(url);
	}
}